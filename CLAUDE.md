...
- **MCP as a local gateway**: when `MCP_ENABLED=true`, an `aggregator_proxy/mcp_server.py` factory builds a `FastMCP.from_fastapi()` sub-app mounted at `MCP_PATH`. Route maps expose only `GET /reservations` and `GET /reservations/{connectionId}`, both as `MCPType.TOOL`; everything else is `MCPType.EXCLUDE`. They are Tools rather than Resources because many MCP clients (e.g. Claude Desktop) only support the tools surface. MCP keeps its own JWT verifier (`fastmcp.JWTVerifier`, configured via the dedicated `MCP_OIDC_*` settings) because MCP access tokens come from a different OIDC provider than the portal IdP. The internal MCP→REST call goes through an httpx event hook (`_forward_user_identity`) that decodes the validated JWT's payload, reads `MCP_OIDC_EMAIL_CLAIM` and `MCP_OIDC_GROUPS_CLAIM`, and sets `X-Auth-Request-Email` + `X-Auth-Request-Groups` on the outgoing request — letting REST trust the same kind of headers as the portal path. `Authorization` is dropped on the internal call. The Settings model validator refuses the combination `PROXY_AUTH_ENABLED=true ∧ MCP_ENABLED=true ∧ MCP_AUTH_ENABLED=false` at startup to prevent unverified JWT claims from being translated into trusted REST headers.
...
aggregator_proxy/
  ...
  models.py             # Pydantic request/response models and ReservationStatus enum.
                        #   P2PS is the shared/response shape; P2PSRequest adds the request-only ero
...
| `SoapFault` | Sync | The provider rejected the operation outright; carries `faultstring` and any `ServiceException` from `<detail>` |
...
- **A SOAP Fault must be parsed, not fall through**: `parse()` has an explicit `case "Fault"`. Before
  it did, a provider rejection hit the catch-all `raise ValueError(f"Unknown NSI operation: …")` and
  surfaced as a 502 naming only the type, with the actual reason discarded — which made a stuck
  multi-domain teardown in dev take far longer to diagnose than it should have. Two namespace traps:
  SOAP 1.1 leaves `faultstring`/`detail` **unqualified** even though `Fault` itself is in the envelope
...
- **The p2ps `<ero>` elements are unqualified**: neither the p2p nor the types XSD declares `elementFormDefault`, so `ero`/`orderedSTP`/`stp` carry no namespace, and the XSD sequence puts `ero` directly after `destSTP`. `test_schema_validation.py` alone will *not* catch a namespace mistake — `P2PServiceBaseType` ends with `<xsd:any namespace="##other" processContents="lax"/>`, so an ero built in the types namespace validates clean and is then silently dropped by safnari. `test_builders.py` asserts the bare tag names for that reason.
...
- **Settled re-delivery is deferred + retried**: the idempotent paths re-deliver via `_redeliver_settled_callback` as a background task (so the 202 returns first) that retries while the requester returns 409. The requester's retried action step is still mid-POST when the result would otherwise arrive — i.e. before its process re-enters `AWAITING_CALLBACK` — and would reject it. Do **not** revert this to an inline `await _send_callback`.
